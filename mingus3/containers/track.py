...
from mingus3.containers.note_container import NoteContainer, UnexpectedObjectError
...
class Track(object):
    ...
    def add_notes(self, note, duration=None):
        ...
        if self.instrument:
            if not self.instrument.can_play_notes(note):
                raise InstrumentRangeError("Note '%s' is not in range of the instrument (%s)"
    ...
    #  Todo: check mingus3.extra.tunings
    ...
    def __setitem__(self, index, value):
        ...
        if not hasattr(value, 'bar'):
            raise UnexpectedObjectError(
                ...
                "mingus.containers.Barobject" % value
            )
