...
chord_shorthand_meaning = {  # Triads Augmented chords Suspended chords Sevenths
    ...
    'hendrix': ' dominant sharp ninth',
    '7b12': ' dominant sharp ninth',
    '5': ' perfect fifth',
}
...
def diminished_seventh(note):
    ...
    ['C', 'Eb', 'Gb', 'Bbb']
    """
    return diminished_triad(note) + [intervals.diminished_seventh(note)]
...
def dominant_sharp_ninth(note, reduce_accidentals=False):
    ...
    """
    res = dominant_ninth(note)
    aug_ninth = notes.augment(intervals.major_second(note))
    if reduce_accidentals:
        aug_ninth = notes.reduce_accidentals(aug_ninth)
    res[4] = aug_ninth
    return res
...
def lydian_dominant_seventh(note):
    ...
    ['C', 'E', 'G', 'Bb', 'F#']
    """
    return dominant_seventh(note) + [intervals.augmented_fourth(note)]
...
def hendrix_chord(note):
    ...
    ['C', 'E', 'G', 'Bb', 'Eb']
    """
    return dominant_sharp_ninth(note)
...
# Todo: allow '0' and 'o' symbol for half-diminished and diminished chord
# Todo: add mM9, mM13, 13#11
def from_shorthand(shorthand_string, slash=None):
    ...
    root_note = shorthand_string[0]

    # Look for accidentals and add them back to root note
    ...
    slash_index = -1
    after_rt = len(root_note)
    rest_of_string = shorthand_string[after_rt:]
    for i, n in enumerate(rest_of_string):
        if n == '/':
            slash_index = i
        elif n == '|':
            # Process polychord
            lower_chord = from_shorthand(shorthand_string[after_rt + i+1:])
            upper_chord = shorthand_string[:after_rt + i]
            return from_shorthand(upper_chord, slash=lower_chord)

    # Process slash chord
    slash_count = rest_of_string.count('/')
    quality_with_slash_count = sum(rest_of_string.count(q)
                                   for q in ('m/M7', '6/9', '6/7'))
    if slash_index != -1 and slash_count > quality_with_slash_count:
        upper_chord, bass = shorthand_string.rsplit('/', 1)
        return from_shorthand(upper_chord, slash=bass)

    chord_quality = shorthand_string[after_rt:]
    if chord_quality in chord_shorthand:
        res = chord_shorthand[chord_quality](root_note)
        if slash:
            # Add slashed chords
            if type(slash) is str:
...
# Todo: ['G','C','E'] is not recognized as second inversion of CM
    ...
    def inversion_exhauster(triad, shorthand, tries, result):
        ...
        if tries != 3 and not no_inversions:
            ...
            for r in result:
                qual, trls, root = r
                if shorthand:
                    res.append(root + qual)
                else:
                    res.append(root + chord_shorthand_meaning[qual] +
                               inv_desc(trls))
            return res
...
chord_shorthand = {  # Triads Augmented chords Suspended chords Sevenths Sixths
    ...
    'hendrix': dominant_sharp_ninth,
    '7b12': dominant_sharp_ninth,
