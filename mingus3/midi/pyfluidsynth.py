...
class Synth:
    ...
    def start(self, driver=None, device=None, midi_driver=None):
        ...
        if driver is not None:
            assert (driver in ['alsa', 'oss', 'jack', 'portaudio', 'sndmgr', 'coreaudio', 'dsound'])
...
class Sequencer:
    ...
    def register_fluidsynth(self, synth):
        ...
        if response == FLUID_FAILED:
            raise RuntimeError("Registering fluid synth failed")
        return response
    ...
    def register_client(self, name, callback, data=None):
        ...
        if response == FLUID_FAILED:
            raise RuntimeError("Registering client failed")
    ...
    def _schedule_event(self, evt, time, absolute=True):
        ...
        if response == FLUID_FAILED:
            raise RuntimeError("Scheduling event failed")
